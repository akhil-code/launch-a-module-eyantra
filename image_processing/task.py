...
* Functions: move, soft_rotate, rotate, find_bot,move_forward, move_backward, pwm_reset, getROI,
            getPositionFromCoord, findBlockColor, getCoord, getxy, find_critical_pts, find_critical_pts_mod, dir_rotate.
* Global Variables: row_margin_top, row_margin_bottom, col_margin_left, col_margin_right, src_img,shapes, colors,
                    centroids, contour_areas, matches, values, cap, cam_delay, no_of_frames, ser_delay, add_delay,
                    soft_flag, cmd_count.

* NOTE: change the COM PORT in the comm.py file(in the 'init_serial' function) when executing the code
* NOTE: certain parameters are adjusted in accordance with the position and height of camera and are commented
* such as '#old:1000', please change these parameters when calibrating
'''
...
# communication module, shortest path algorithm module, finds the angle (module)
import comm, algo_mod, angle_module
...
def getxy(pos, img):
    ...
    if c == 1 or c == 9:
        if c == 1:
            cols_start, cols_end = 0, w_s
        elif(c ==9):
            cols_start, cols_end = cols - w_s, cols
    else:
    ...
    if r == 1 or r == 9:
        if r == 1:
            rows_start, rows_end = 0, h_s
        else:
            rows_start, rows_end = rows - h_s, rows
    else:
...
def move(dis_px):
    ...
    comm.send_data('<d')
    cv2.waitKey(ser_delay)

    while length > 1:
        d = int(dis_cm / math.pow(10, length-2))
        dis_cm = dis_cm % math.pow(10, length-2)
        comm.send_data(str(d))
        cv2.waitKey(ser_delay)
        length -= 2
    if dis_cm != 0:
        comm.send_data(str(int(dis_cm)))
        cv2.waitKey(ser_delay)
    comm.send_data('>')
    cv2.waitKey(ser_delay)
...
def soft_rotate(ang):
    ...
    if ang > 99:
        ...
        comm.send_data(str(d))
        cv2.waitKey(ser_delay)
        comm.send_data(str(n))
        cv2.waitKey(ser_delay)
    else:
        comm.send_data(str(ang))
        cv2.waitKey(ser_delay)
...
def find_bot(capf, forward_values, backward_values):
    ...
    for dir in range(2):
        if dir == 0:
            low_values, high_values = forward_values[0], forward_values[1]
        else:
            low_values, high_values = backward_values[0], backward_values[1]
...
def getROI(img, pos):
    ...
    if c == 1 or c == 9:
        ...
        cols_start, cols_end = (c-2)*w_l + w_s, (c-1)*w_l + w_s
