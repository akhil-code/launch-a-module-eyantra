...
*Functions:find_route,delete_point,insert_point,ext_route,graph_dist,minDistance,set_objects_positions,get_objects_positions,set_free_loc,get_neighbours,dijkistra,find_critical_pts,
            findAllPaths,findAllPathsUtil
...
'''
*Function Name: findAllPathsUtil
*Input: point1,point2,visited array, path array, shortest length
*Output: finds the multiple paths available with the shortest distance
*Logic: iterates recurrsively to find the multiple paths using parentx array
*Example Call:findAllPathsUtil(1,5,[],[],4)
'''
def findAllPathsUtil(ux, dx, visitedx, pathx,short_len):
    ...
    if ux == dx and len(pathx) == short_len:
        ...
        for i in parentx[ux]:
            if visitedx[i]== -1:
                findAllPathsUtil(i, dx, visitedx, pathx,short_len)
...
'''
*Function Name: findAllPaths
*Input: source point,destination point,shortest length
*Output: finds the multiple paths available with the shortest distance
*Logic: makes use of the findAllPathsUtil to iterate recurrsively
*Example Call:findAllPath(1,5,4)
'''  
# Prints all paths from 's' to 'd'
def findAllPaths(sx, dx,short_len):
    global pathx_list
    ...
    findAllPathsUtil(sx, dx,visitedx, pathx,short_len)
...
parent = []
#parentx and pathx_list are used to find the multiple paths from source to destination of shortest length
parentx = []
pathx_list = []
...
#using algo module
#step1: call set_objects_positions(arr[])
#step2: call find_route(source,destination,details_src(color,shape),details_dest(color,shape))
#step3:call set_free_loc to add that point of the arena i.e free space
