...
#trackbars used to control the blur
cv2.namedWindow('pallete',cv2.WINDOW_AUTOSIZE)
cv2.createTrackbar('gaussian','pallete',0,1,nothing)
cv2.createTrackbar('bilateral','pallete',0,1,nothing)
cv2.createTrackbar('bilateral_blur','pallete',9,20,nothing)
cv2.createTrackbar('gauss_blur','pallete',7,20,nothing)

#flags representing whether the blur has to be used or not
gaussianFlag = 0
bilateralFlag = 0
...
#implementing trackbars to control the blur applied on the image captured
while(True):
    ...
    gaussianFlag = cv2.getTrackbarPos('blur','pallete')
    bilateralFlag = cv2.getTrackbarPos('blur','pallete')
    gauss_blur = cv2.getTrackbarPos('gauss_blur','pallete')
    bilateral_blur = cv2.getTrackbarPos('bilateral_blur','pallete')

    if(gaussianFlag == 1):
        img  = cv2.bilateralFilter(img,bilateral_blur,75,75)
    if(bilateralFlag == 1):
...
for x in (1,10,19,28,37,46):#make the values -1
    for y in range(1,55):
        ...
        if( x != y and shapes[x] != -1 and colors[x]!=-1 and shapes[x] == shapes[y] and colors[x] == colors[y]):
            if(y%9 != 1 and abs(contour_areas[y] - contour_areas[x]) < 90):
                #matches.append((y,x))
                if(matches[(x/9)+1] == -1):
                    matches[(x/9) + 1] = y
                else:
...
no_of_frames = 12           #no of frames that have to be captured in order to empty the buffer
ser_delay = 20 #old:40,30,20              #delay between successive bytes when communicating with xbee
add_delay = 0
soft_flag = 0
...
forward_values = ((102,59,196),(180,142,255))
backward_values = ((3,84,164),(47,152,255))
